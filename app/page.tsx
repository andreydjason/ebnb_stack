...
export default async function Inicio() {
  ...
  return (
    <FakeLoader>
      <div>
        <p className="flex"></p>
        <div className="mt-0 mx-auto flex max-w-sm items-center gap-x-4 rounded-xl bg-white p-6 shadow-lg outline outline-black/5 dark:bg-slate-800 dark:shadow-none dark:-outline-offset-1 dark:outline-white/10">
        ...
        <div className="space-y-6">
          {articles.map((article) => (
            <div key={article.slug} className="border p-4 rounded-lg bg-white dark:bg-gray-800 dark:text-white">
              ...
              <p className="text-gray-500 dark:text-gray-300 text-sm mt-1">{new Date(article.date).toLocaleDateString('pt-BR', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
              <p className="text-gray-700 mt-2 dark:text-white">{article.summary}</p>
            </div>
          ))}
        </div>
      </div>
